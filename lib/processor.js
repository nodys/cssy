...
function processor(filename) {
  ...
  return function(src, callback) {
    ...
    var rw      = rework(src, {'source':relpath});


    // Ressource filename
    rw.filename = filename;

    // Uniq ressource id based on filename (relative to cwd())
    ...
    // Global config:
    rw.globalConfig = (process.cssy && process.cssy.config) ? process.cssy.config : {};

    globalPreProcessor(rw, function(err, rw) {
      ...
      // Expose other imported ressources
      rw.imports  = imports;

      // Override default rework toString method
      ...
      // Post processing (using global process)
      globalPostProcessor(rw, callback)

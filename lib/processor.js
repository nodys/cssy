  ...
  return function(css, done) {
    ...
    async.waterfall([
      ...
      function(ctx, next) {
        ...
        if(getGlobalConfig().sourcemap) {
          ctx.css  = styles.toResult({ map: { inline : true } }).css;
          ctx.css +=  '\n/*# sourceURL=' + relpath + '.output'   + '*/';
        } else {
          ctx.css  = styles.toResult({ map: false }).css;
        }
